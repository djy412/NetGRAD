# -*- coding: utf-8 -*-
"""
Created on Sat Oct  4 19:40:39 2025
NetVLAD Implementation for Comparison
@author: djy41
"""
...
class CFG:
    ...
    # NetVLAD settings
    vlad_clusters: int = 64        # typical: 64
    vlad_dim: int = 512            # VGG-16 conv5_3 channels
    vlad_alpha: float = 100.0      # assignment softness (Arandjelovic et al.)
    normalize_input: bool = True

    # KMeans for cluster init (sampled from conv maps)
    kmeans_samples: int = 100000   # number of local descriptors for k-means
    ...
    batch_size: int = 32
    num_workers: int = 8
    epochs_ft: int = 8             # fine-tune VGG+VLAD with triplet
    ...
    warmup_lr_backbone: float = 1e-3
    warmup_lr_head: float = 1e-3
    warmup_proj_dim: int = 2048    # set 0 to disable projection (use raw 512-D)
...
# =========================
# Helpers re-used from your style
# =========================
@torch.no_grad()
...
class NetVLAD(nn.Module):
    """Standard NetVLAD layer.
    Based on: "NetVLAD: CNN architecture for weakly supervised place recognition" (CVPR 2016).
    """
    def __init__(self, num_clusters: int, dim: int, alpha: float = 100.0, normalize_input: bool = True):
        ...
        self.clusters = nn.Parameter(torch.rand(self.K, self.D))         # C_k
        self.conv = nn.Conv2d(self.D, self.K, kernel_size=(1, 1), bias=True)  # assignment
    ...
    def init_params(self, centroids: torch.Tensor):
        """Set cluster centers and assignment conv from k-means centroids.
        "w_k = 2*alpha*c_k" and "b_k = -alpha*||c_k||^2" (from the paper's soft-assignment derivation).
        """
        ...
        # init assignment conv
        w = 2.0 * self.alpha * self.clusters  # [K, D]
        b = - self.alpha * (self.clusters.pow(2).sum(dim=1))  # [K]
    ...
    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        x: [B, D, H, W]
        Returns (vlad, assign):
          vlad: [B, K*D] L2-normalized NetVLAD descriptor
          assign: [B, K, H, W] soft-assignments (optional for viz)
        """
        B, C, H, W = x.shape
        assert C == self.D

        if self.normalize_input:
            x = F.normalize(x, p=2, dim=1)  # across channels at each spatial location

        # Soft assignments
        soft = self.conv(x)                           # [B, K, H, W]
        soft = F.softmax(soft.view(B, self.K, -1), dim=2).view(B, self.K, H, W)

        # Residuals to each cluster center
        x_flat = x.view(B, C, -1)                     # [B, D, HW]
        soft_flat = soft.view(B, self.K, -1)          # [B, K, HW]

        # compute residuals sum over HW per cluster
        vlad = torch.zeros([B, self.K, C], dtype=x.dtype, device=x.device)
        for k in range(self.K):
            # sum over HW: soft_k * (x - c_k)
            soft_k = soft_flat[:, k, :].unsqueeze(1)           # [B,1,HW]
            x_res = x_flat - self.clusters[k:k+1].unsqueeze(-1)  # [B,D,HW]
            vlad[:, k, :] = (soft_k * x_res).sum(dim=-1)       # [B,D]

        # intra-normalization
        vlad = F.normalize(vlad, p=2, dim=2)                    # per cluster
...
# =========================
# VGG-16 backbone up to conv5_3
# =========================
# class VGG16_Backbone(nn.Module): #--- Uses pretrained weights
...
#         # up to conv5_3 (layer indices 0..30). VGG features: 0..30 conv blocks; 30 is ReLU after conv5_3
#         self.backbone = nn.Sequential(*features[:31])
...
# replace your VGG16_Backbone with this version for RGB images
...
#     def forward(self, x):
#         # If grayscale, replicate to 3 channels (B,1,H,W) -> (B,3,H,W)
#         if x.size(1) == 1:
#             x = x.expand(-1, 3, -1, -1)
#         return self.backbone(x)
...
class VGG16_NetVLAD(nn.Module):
    # def __init__(self, K: int = 64, alpha: float = 100.0, normalize_input: bool = True,
    ...
    # def __init__(self, K, alpha, normalize_input, train_backbone=False, pretrained_backbone=True):
    #     super().__init__()
    #     self.backbone = VGG16_Backbone(pretrained=pretrained_backbone,
    #                                    requires_grad=train_backbone)
    #     self.netvlad  = NetVLAD(num_clusters=K, dim=512, alpha=alpha,
    #                             normalize_input=normalize_input)
    def __init__(self, K, alpha, normalize_input, train_backbone=False,
                 pretrained_backbone=True, backbone: Optional[nn.Module]=None):
        super().__init__()
        # reuse warmed backbone if provided
        self.backbone = backbone if backbone is not None else \
                        VGG16_Backbone(pretrained=pretrained_backbone, requires_grad=train_backbone)
        self.netvlad  = NetVLAD(num_clusters=K, dim=512, alpha=alpha, normalize_input=normalize_input)
    ...
    def forward(self, x: torch.Tensor):
        ...
        desc, assign = self.netvlad(feat)       # [B,K*D], [B,K,h,w]
        return desc, assign
...
class VGG16_WarmupGeM(nn.Module):
    """Backbone + GeM (+ optional projection) → L2 descriptor for a brief warmup."""
    def __init__(self, backbone: nn.Module, proj_dim: int = 2048, normalize_input: bool = True):
        super().__init__()
        self.backbone = backbone
        self.normalize_input = normalize_input
        self.gem = GeM(p=3.0, learn_p=True)
        self.proj = nn.Linear(512, proj_dim) if proj_dim and proj_dim > 0 else None
    def forward(self, x):
        ...
        if self.proj is not None:
            g = self.proj(g)                 # [B,proj]
        return F.normalize(g, dim=1)

def train_warmup_backbone(model: VGG16_WarmupGeM, trip_loader: DataLoader, device: str):
    """Short triplet warmup so conv features become meaningful before NetVLAD k-means."""
    model.train()
    # backbone params + head params with distinct LRs
    params = [
        {"params": [p for n,p in model.named_parameters() if "backbone" in n and p.requires_grad],
         "lr": cfg.warmup_lr_backbone},
        {"params": [p for n,p in model.named_parameters() if "backbone" not in n],
         "lr": cfg.warmup_lr_head},
    ]
    opt = torch.optim.Adam(params)
    for ep in range(cfg.warmup_epochs):
        t0 = time.time(); losses = []
        for anchor, pos, neg, *_ in trip_loader:
            a = anchor.to(device, non_blocking=True)
            p = pos.to(device, non_blocking=True)
            n = neg.to(device, non_blocking=True)
            Da = model(a); Dp = model(p); Dn = model(n)
            loss = triplet_cosine(Da, Dp, Dn, margin=cfg.triplet_margin)
            opt.zero_grad(); loss.backward(); opt.step()
            losses.append(float(loss.item()))
        print(f"[WarmupGeM {ep+1}/{cfg.warmup_epochs}] loss={np.mean(losses):.4f} time={time.time()-t0:.1f}s")

# =========================
# KMeans init on conv features
# =========================
@torch.no_grad()
def collect_local_descriptors(backbone: nn.Module, loader: DataLoader, device: str,
                              max_desc: int = 100000) -> np.ndarray:
    """Collect random local descriptors (conv5_3 features) for k-means.
    ...
    for imgs, _ in loader:
        ...
        B, C, H, W = fmap.shape
        # L2-normalize per-location to match NetVLAD normalize_input
        fmap = F.normalize(fmap, p=2, dim=1)
        fmap = fmap.permute(0, 2, 3, 1).contiguous().view(-1, C)  # [B*H*W, 512]
        fmap_np = fmap.detach().cpu().numpy()
        pool.append(fmap_np)
        total += fmap_np.shape[0]
        if total >= max_desc:
            break
...
def init_clusters_kmeans(backbone: nn.Module, train_loader: DataLoader, device: str,
                         K: int, max_desc: int) -> torch.Tensor:
    X = collect_local_descriptors(backbone, train_loader, device, max_desc=max_desc)
    print(f"KMeans on {len(X)} local descriptors ...")
...
# =========================
# Training (triplet on NetVLAD descriptors)
# =========================
...
def train_triplet(model: VGG16_NetVLAD, trip_loader: DataLoader, device: str):
    model.train()
    params = [
        {"params": [p for n,p in model.named_parameters() if "backbone" in n and p.requires_grad], "lr": cfg.lr_backbone},
        {"params": [p for n,p in model.named_parameters() if "netvlad" in n], "lr": cfg.lr_vlad},
    ]
    opt = torch.optim.Adam(params)
    ...
    for ep in range(cfg.epochs_ft):
        ...
        for anchor, pos, neg, *_ in trip_loader:
            ...
            loss = triplet_cosine(Da, Dp, Dn, cfg.triplet_margin)
            opt.zero_grad(); loss.backward(); opt.step()
            losses.append(loss.item())
        print(f"[Fine-tune {ep+1}/{cfg.epochs_ft}] loss={np.mean(losses):.4f} time={time.time()-t0:.1f}s")
    
    torch.save(model.state_dict(), "weights/NetVLAD_Model.pth")
    print("Model saved ✅")
    
# =========================
# Descriptor extraction & evaluation
# =========================
@torch.no_grad()
...
if __name__ == "__main__":
    ...
    # # ---- Build model ----
    model = VGG16_NetVLAD(K=cfg.vlad_clusters, alpha=cfg.vlad_alpha, normalize_input=cfg.normalize_input,
                               train_backbone=False).to(device)

    # #--- NetVLAD from scratch (random VGG16)
    # model = VGG16_NetVLAD(
    #     K=cfg.vlad_clusters,
    #     alpha=cfg.vlad_alpha,
    #     normalize_input=cfg.normalize_input,
    #     train_backbone=True,            # <— train the backbone
    #     pretrained_backbone=False       # <— random init
    # ).to(device)

    # # Make the backbone LR larger when training from scratch
    # cfg.lr_backbone = 1e-3
    # cfg.lr_head     = 1e-3
    # cfg.epochs_ft   = 30          # ~10–50; tune per dataset

    # ---- Build a shared backbone (from scratch) ----
    backbone = VGG16_Backbone(pretrained=True, requires_grad=True).to(device)
    
    # # ---- 1) Warmup with GeM ----
    # print("==> Warmup VGG16 with GeM (triplet)")
    # warmup_head = VGG16_WarmupGeM(backbone=backbone,
    #                               proj_dim=cfg.warmup_proj_dim,
    #                               normalize_input=cfg.normalize_input).to(device)
    # train_warmup_backbone(warmup_head, trip_loader, device)
    
    # # (Optional) after warmup, you can reduce the backbone LR for the NetVLAD stage:
    # cfg.lr_backbone = 3e-4
    # cfg.lr_vlad     = 1e-3
    # cfg.epochs_ft   = 20
    
    # # ---- 2) KMeans init using the warmed backbone ----
    # print("==> KMeans init clusters (conv features, warmed)")
    # with torch.no_grad():
    #     C0 = init_clusters_kmeans(backbone, train_loader, device=device,
    #                               K=cfg.vlad_clusters, max_desc=cfg.kmeans_samples)
    
    # ---- 3) NetVLAD model reusing the *same* warmed backbone ----
    # model = VGG16_NetVLAD(
    #     K=cfg.vlad_clusters,
    #     alpha=cfg.vlad_alpha,
    #     normalize_input=cfg.normalize_input,
    #     train_backbone=True,            # keep training the backbone (smaller LR)
    #     pretrained_backbone=False,
    #     backbone=backbone               # <— reuse warmed convs
    # ).to(device)
    # #model.init_netvlad(C0)
    
    # print("==> Fine-tune VGG+NetVLAD on triplets")
    # train_triplet(model, trip_loader, device)

    # ---- KMeans init for NetVLAD ----
    print("==> KMeans init clusters (conv features)")
    with torch.no_grad():
        C0 = init_clusters_kmeans(model.backbone, train_loader, device=device, K=cfg.vlad_clusters, max_desc=cfg.kmeans_samples)
        model.init_netvlad(C0)

    # ---- Optional fine-tuning with triplet ----
    if Global_Train:
        print("==> Fine-tune VGG+NetVLAD on triplets")
        train_triplet(model, trip_loader, device)
    else: #--- Load models weights
        print("Loading Model wieghts from Previous Training")  
        load_model_path = 'C:/Users/djy41/Desktop/PhD Work/Code/C_2) Visual Place Recognition/weights/NetVLAD_Model.pth'    
    ...
    # plot_tsne(db_desc, db_lbls, max_points=3000, save_path="runs/vis/netvlad_tsne_db.png",
    #           title="t-SNE (VGG16+NetVLAD DB descriptors)")
    ...
    #                        K=5, save_dir="runs/vis", name_prefix="netvlad_topk")
    # print("Saved NetVLAD galleries and plots to runs/vis/")
    
    
    
    if EVAL_Live_Stream:
        Dvlad = model.netvlad.K * model.netvlad.D
        ...
        TAU_CENTER = 0.68     #--- How close the current frame needs to be to another cluster center to be considered for revisit
        ...
        memory = OnlinePlaceMemory(dim=Dvlad, tau_assign=TAU_CENTER, ema=0.1)
        
        params = list(model.netvlad.parameters())  # or [] to disable online learning
